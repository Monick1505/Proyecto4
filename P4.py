...
# Para realizar esta simulación inicialmente se importan librerias de interes
...
# Además se definen las funciones ya brindadas en el complemento teorico del proyecto. 
...
# Función para codificar los pixeles a bits
def rgb_a_bit(imagen):
    '''Convierte los pixeles de base 
    decimal (de 0 a 255) a binaria 
    (de 00000000 a 11111111).
...
# Función para simular un canal ruidoso que afecta la señal
def canal_ruidoso(senal_Tx, Pm, SNR):
    '''Un bloque que simula un medio de trans-
    misión no ideal (ruidoso) empleando ruido
    AWGN. Pide por parámetro un vector con la
...
# Función para pasar de bits a RGB
def bits_a_rgb(bits_Rx, dimensiones):
    '''Un blque que decodifica el los bits
    recuperados en el proceso de demodulación
...
## En esta sección 4.1 se realizarán modificaciones a la funciones para modular la señal. ##
# Estas funciones forman un modulado BPSK y se ajustarán para modulado QPSK

# Nueva función del modulador QPSK

def qpsk_mod(bits, fc, mpp):
    '''Un método que simula el esquema de 
    ...
    #Una diferencia de BPSK y QPSK es que para la modulación QPSK se utilizará dos portadoras
    ...
    # Hasta este punto la función es muy parecida a la BPSK pero considerando el criterio la s(t)
    # Y el funcionamieto de la modulación QPSK  se define un nuevo bucle en función de la modulación QPSK
    
    # 4. Bucle para asignar las formas de onda:
    # Se tiene: 
    # A1 = -1 if b1 = 0
    # A1 = 1 if b1 = 1
    # A2 = 1 if b2 = 0
    ...
    # Una vez concluido el bucle se pueden sumar las portadoras por la propidad de ortoganilidad en la demodulación QPSK
...
# La función para demodular la señal tambien se define pero se puede utilizar la brindada igual

# La función demoduladora QPSK
def demodulador(senal_Rx, portadora, mpp):
    '''Un método que simula un bloque demodulador
...
# Ahora se inicia la asignación de valores a las variables e invocacion de funciones para ejecutar el código

# Se definen los parámetros establecidos para la simulación
fc = 5000  # frecuencia de la portadora
mpp = 20   # muestras por periodo de la portadora
SNR = 5    # relación señal-a-ruido del canal

# Se utiliza el codigo de la sección 3.2 para que la simulación se realice de forma similar
...
senal_Tx, Pm, portadora, moduladora = qpsk_mod(bits_Tx, fc, mpp)
...
""" 4.2 """
# En esta sección se busca realizar pruebas de estacionaridad y ergodicidad a la señal modulada senal_Tx

# Inicialmente se define una variable a la que se le asigna el tiempo de muestra
time = np.linspace(0, 0.1,100)



# Por el formato qpsk hay cuatro funciones de tiemplo f(t)
Ft = np.empty((4, len(time)))	   # 4 funciones del tiempo f(t) 

# Se sabe que los valores de A1 y A2 son 1 y -1
A=[1,-1]
 
plt.figure()

# Se crea un bucle para definir una matris con los valores posibles   
for i in A:
...
# Se define una variable para determinar el promedio de las cuatro realizaciones en cada instante
...
# El gráfico generado muestra que el promedio de realizaciones y el valor teórico esperado tienen el mismo valor
# Por lo que se puede decir que la señal posee ergodicidad. 


""" 4.3 """
# En esta sección se busca determinar y gráficar la densidad espectral de potencia para la señal modulada. 

# Inicialmente se importa la libreria fft para poder aplicar la transformada de Fourier
from scipy import fft

#Con esto la realizacion del proceso es equivalente a la senal_Tx
# Se aplica la transformada de Fourier a la señal 
tfsenal = fft(senal_Tx)

# Posteriormete se tiene el periodo de muestreo time_m= Tc/mpp = (1/fc) /mpp
time_m = (1/fc) / mpp

# Se genera una variable con el número de entradas de la señal_Tx (para simplicidad)
muestras = len(senal_Tx)

# Se define el eje horizontal del gráfico, en este caso corresponde al espacio de frecuencias
